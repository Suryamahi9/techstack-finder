...
export async function GET(request) {
  ...
  <rect x="10" y="10" width="48" height="48" rx="10" fill="${bg2}" stroke="${border}" stroke-width="0.5"/>
  <image href="${faviconUrl}" x="14" y="14" width="40" height="40" rx="8"/>
  ...
  if (format === 'json') {
    return NextResponse.json({
      badge: {
        svg: badgeSvg,
        embed: `<a href="${siteUrl}" target="_blank" rel="noopener"><img src="https://techstack-finder.vercel.app/api/badge?url=${encodeURIComponent(hostname)}&theme=${theme}" alt="Built with tech detected by TechStack Finder" /></a>`,
        markdown: `[![TechStack Finder](https://techstack-finder.vercel.app/api/badge?url=${encodeURIComponent(hostname)}&theme=${theme})](${siteUrl})`,
        html: `<a href="${siteUrl}" target="_blank" rel="noopener"><img src="https://techstack-finder.vercel.app/api/badge?url=${encodeURIComponent(hostname)}&theme=${theme}" alt="Built with tech detected by TechStack Finder" /></a>`,
      },
    });
  }
