  ...
  return (
    <div className="rounded-2xl border border-border bg-elevated p-6 animate-fade-up">
      ...
      <div className="flex gap-2 mb-4">
        ...
        <span className="inline-flex items-center gap-1 rounded-full bg-yellow-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-yellow-400">
        ...
        {costEstimate.items.map((item, i) => (
          <div key={i} className="flex items-center justify-between rounded-xl border border-border bg-bg/50 px-3 py-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className={`font-mono text-xs font-bold ${item.baseCost === 0 ? 'text-emerald-400' : 'text-muted'}`}>
