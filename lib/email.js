...
export async function sendContactMessage({ name, email, subject, message }) {
  ...
  const res = await resend.emails.send({
    from: FROM,
    to: process.env.CONTACT_TO || FROM,
    replyTo: email,
    subject: `Contact form: ${subject || 'Website enquiry'}`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:32px;">
        <h1 style="font-size:20px;margin-bottom:16px;">New contact form submission</h1>
        <p style="color:#555;line-height:1.6;"><strong>Name:</strong> ${name || '—'}</p>
        <p style="color:#555;line-height:1.6;"><strong>Email:</strong> ${email || '—'}</p>
        <p style="color:#555;line-height:1.6;"><strong>Subject:</strong> ${subject || '—'}</p>
        <div style="background:#f8f8f8;border-radius:8px;padding:16px;margin:20px 0;color:#333;white-space:pre-wrap;">${message}</div>
      </div>
    `,
  });
  return res;
...
export async function sendAffiliateApplication({ name, email, website, audience }) {
  if (!resend) return null;
  const res = await resend.emails.send({
    from: FROM,
    to: process.env.CONTACT_TO || FROM,
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:32px;">
        <h1 style="font-size:20px;margin-bottom:16px;">New affiliate application</h1>
        <p style="color:#555;line-height:1.6;"><strong>Name:</strong> ${name || '—'}</p>
        <p style="color:#555;line-height:1.6;"><strong>Email:</strong> ${email || '—'}</p>
        <p style="color:#555;line-height:1.6;"><strong>Website / social:</strong> ${website || '—'}</p>
        <p style="color:#555;line-height:1.6;"><strong>Audience:</strong> ${audience || '—'}</p>
      </div>
    `,
  });
  return res;
