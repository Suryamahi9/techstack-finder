...
export async function browserScanRemote(url, timeoutMs = 30000) {
  ...
  if (!apiKey) {
    console.warn('[browserScanRemote] BROWSERLESS_API_KEY not set');
    return null;
  }

  try {
    // Correct Browserless.io cloud endpoint
    const browserlessUrl = `https://chrome.browserless.io/content?token=${apiKey}&timeout=${timeoutMs}&waitUntil=networkidle2&blockAds=true&blockResources=font,media`;
    console.log('[browserScanRemote] Requesting:', url);
    ...
    if (!response.ok) {
      ...
      console.warn('[browserScanRemote] Browserless error:', response.status, errText);
      return null;
    }

    const html = await response.text();
    console.log('[browserScanRemote] Got HTML length:', html?.length || 0);
    if (!html || html.length < 500) {
      console.warn('[browserScanRemote] HTML too short:', html?.substring(0, 200));
      return null;
    }
