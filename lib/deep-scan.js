...
export async function fetchCssJsResources($, origin) {
  const cssUrls = [];
  const jsUrls = [];
  ...
  const cssPromises = cssUrls.slice(0, 8).map(async (url) => {
    const r = await safeFetch(url, 6000);
    if (r && r.text) cssContent.push(r.text);
  });

  const jsPromises = jsUrls.slice(0, 8).map(async (url) => {
    const r = await safeFetch(url, 6000);
...
export async function probePaths(origin) {
  const results = {};
  ...
  const probes = PROBE_PATHS.map(async (path) => {
    ...
    try {
      ...
      const res = await fetch(url, { ...FETCH_OPTS, signal: ctrl.signal });
      clearTimeout(tid);
      results[path] = { status: res.status, contentType: res.headers.get('content-type') || '' };
    } catch {
      results[path] = { status: 0, contentType: '' };
    }
  });
...
export function buildPathProbesSummary(probes) {
  ...
  for (const [path, result] of Object.entries(probes)) {
    if (result.status === 200) {
      summaries.push(`${path} -> ${result.status}`);
    } else if (result.status >= 300 && result.status < 400) {
      summaries.push(`${path} -> ${result.status} redirect`);
    } else if (result.status === 401 || result.status === 403) {
      summaries.push(`${path} -> ${result.status} protected`);
    }
  }
